/* 미로 생성 · 경로 · 시야 — 순수 함수만 둔다.
   ...
   좌표 규약은 클라이언트와 같다 (구현계획.md S0):
     ...
     DIRS = [북, 동, 남, 서] — protocol.ts 의 Dir 인덱스와 일치해야 한다 */

import type { Cell, Dir, Grid } from '../shared/protocol.ts';

export const DIRS: ReadonlyArray<{ dx: number; dy: number }> = [
  { dx: 0, dy: -1 }, { dx: 1, dy: 0 }, { dx: 0, dy: 1 }, { dx: -1, dy: 0 },
];

/** 시드 난수 — 같은 시드는 같은 미로를 준다.
    테스트가 재현 가능해야 하고, 방에 접속한 전원이 똑같은 미로를 봐야 한다. */
...
export const walkable = (grid: Grid, x: number, y: number): boolean =>
  y >= 0 && y < grid.length && x >= 0 && x < grid[y]!.length && grid[y]![x] !== '#';

export interface Maze { grid: Grid; w: number; h: number; seed: number }
...
export function generate(w: number, h: number, seed: number): Maze {
  ...
  while(stack.length){
    ...
    const cand: { nx: number; ny: number; d: { dx: number; dy: number } }[] = [];
    for(const d of DIRS){
...
export function bfs(grid: Grid, start: Cell): number[][] {
...
export const reachable = (grid: Grid, a: Cell, b: Cell): boolean => bfs(grid, a)[b.y]![b.x]! >= 0;

export function cells(grid: Grid): Cell[] {
  const out: Cell[] = [];
...
/** from 에서 가장 먼 통로 칸 */
export function farthest(grid: Grid, from: Cell): Cell & { d: number } {
  const dist = bfs(grid, from);
  let best = { x: from.x, y: from.y, d: 0 };
  for(const c of cells(grid)){
    const v = dist[c.y]![c.x]!;
    if(v > best.d) best = { x: c.x, y: c.y, d: v };
  }
  return best;
}
...
export function placeSpawns(grid: Grid, seed: number, itCount = 2): Spawns {
...
export function lineOfSight(grid: Grid, from: Cell, dir: Dir, maxCells = 12): (Cell & { d: number })[] {
  const d = DIRS[dir]!, seen: (Cell & { d: number })[] = [];
